...
const $WithPadding = Symbol('[[WithPadding]]');
const $IgnorePadding = Symbol('[[IgnorePadding]]');
const $AllowConcatenation = Symbol('[[AllowConcatenation]]');

const IsBase32CharCode = charCode => (
  charCode >= 0x30 && charCode <= 0x39 ||
  charCode >= 0x41 && charCode <= 0x5a ||
  charCode >= 0x61 && charCode <= 0x7a
);

const IsBase32Encoding = argument => IsObject(argument) && HasSlot(argument, $Alphabet);

const RequireThis = argument => {
  if (!IsBase32Encoding(argument)) {
    throw new TypeError('`this` is not an instance of Base32Encoding');
  }
}
...
class Base32Encoding {
  ...
  get withPadding() {
    RequireThis(this);
    return GetSlot(this, $WithPadding);
  }

  get ignorePadding() {
    RequireThis(this);
    return GetSlot(this, $IgnorePadding);
  }

  get allowConcatenation() {
    RequireThis(this);
    return GetSlot(this, $AllowConcatenation);
  }

  encode(string, options) {
    RequireThis(this);
    const $string = ToString(string);
    let withPadding = GetSlot(this, $WithPadding);
    if (options !== undefined) {
      RequireOptionsObject(options);
      const $withPadding = options.withPadding;
      if ($withPadding !== undefined) {
  ...
  decode(encodedString, options) {
    ...
    let ignorePadding = GetSlot(this, $IgnorePadding);
    let allowConcatenation = GetSlot(this, $AllowConcatenation);
    if (options !== undefined) {
      RequireOptionsObject(options);
      ...
      const $allowConcatenation = options.allowConcatenation;
      if ($allowConcatenation !== undefined) {
        allowConcatenation = !!$allowConcatenation;
      }
    }
...
exports.default = Base32Encoding;

ReflectDefineProperty(exports, '__esModule', {
  value: true
});

ReflectDefineProperty(Base32Encoding, SymbolHasInstance, {
  value: IsBase32Encoding
});

const Base32EncodingPrototype = Base32Encoding.prototype;

ReflectDefineProperty(Base32EncodingPrototype, SymbolToStringTag, {
  value: 'Base32Encoding'
});
...
if (BigInt) {
  ...
  ObjectDefineProperties(Base32EncodingPrototype, {
    encodeBigInt: {
...
const BASIC = new Base32Encoding('ABCDEFGHIJKLMNOPQRSTUVWXYZ234567');
exports.BASIC = BASIC;
const HEX = new Base32Encoding('0123456789ABCDEFGHIJKLMNOPQRSTUV');
exports.HEX = HEX;

ObjectDefineProperties(Base32Encoding, {
  BASIC: {
    value: BASIC
  },
  HEX: {
